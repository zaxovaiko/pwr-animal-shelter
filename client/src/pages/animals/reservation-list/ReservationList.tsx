  ...
  const columns = [
    {
      ...
      minWidth: 40,
      Cell: (row: any) => (
        <p className={styles_main["table-text"]}>{row.value}</p>
      ),
    },
    {
      ...
      Cell: (row: { original: { date: any } }) => {
        ...
        if (!date) return "";
        return (
          <p className={styles_main["table-text"]}>
            {date.substring(0, 10)} ,g. {date.substring(11, 16)}
          </p>
        );
      },
    },
    {
      ...
      Cell: (row: { original: { date: any } }) => {
        ...
        return (
          <p className={styles_main["table-text"]}>
            {date_to.getFullYear()}-{date_to.getMonth() + 1}-{date_to.getDate()}
          </p>
        );
      },
    },
    {
      ...
      Cell: (row: any) => (
        <p className={styles_main["table-text-comb"]}>
          Chip code:{" "}
          <a
    ...
    {
      Header: () => (
        <strong className={styles_main["table-header"]}>Anulowanie</strong>
      ),
      accessor: "id",
      Cell: (row: any) => {
        if (row.original.reservation_status.value !== "Anulowana") {
          return (
            <p
              ...
              className={styles_main["table-cell-set"]}
            >
              Anuluj rezerwacje
            </p>
          );
